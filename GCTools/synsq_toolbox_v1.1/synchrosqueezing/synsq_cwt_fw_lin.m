...
%
% 1. E. Brevdo, N.S. FuÄ?kar, G. Thakur, and H-T. Wu, "The
...
%---------------------------------------------------------------------------------
function [Tx, fs, Wx, as, w] = synsq_cwt_fw_lin(t, x, nv, opt)
...
if (opt.dtype)
    ...
    [Wx,as,dWx] = cwt_fw_lin(x, opt.type, nv, dt, opt);
    ...
    [Wx,as] = cwt_fw_lin(x, opt.type, nv, dt, opt);
    Wx = Wx(:, n1-4+1:n1+N+4);
...
[Tx,fs] = synsq_cwt_squeeze_lin(Wx, w, t, nv, opt);

Wx=flipud(Wx);
% add zero frequency
wx0=mean(x)*ones(1,size(Wx,2));
Wx=[wx0;Wx];
tx0=mean(x)*ones(1,size(Tx,2));
Tx=[tx0;Tx];
fs=[0 fs];
