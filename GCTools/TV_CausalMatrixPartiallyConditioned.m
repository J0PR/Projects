function [mres,freqs,p]=TV_CausalMatrixPartiallyConditioned(data,optStruct)
...
for drive=1:nvar
    for target=1:nvar
        if drive ~= target
            ...
            conz = conz(1:nd);
            temp_data=[Y(drive,:);Y(target,:);Y(conz,:)];
            
            [tempRes,nullStruct] = TV_ProcessData(temp_data,optStruct,[]);
            if ~isempty(tempRes)
