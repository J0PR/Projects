...
L=zeros(neqs);
l=0; betam=[1  beta(nparma+1:end)];
for i=1:neqs
 cont=neqs-i+1; ind=l+1:l+cont;
...
if r>0
 ...
 bet=Yx1\yfvec; m1=bet(nbeta+1:end);    %estimated initial state
 yfvec=yfvec-rx1*m1;                    %subtract estimated initial state
 yx=reshape(yfvec,neqs,nobs); y=yx';    %transform from vector to matrix
else
 Bs=[]; Ds=[];
end
...
Sigma=dlyapsq(T,HH); Sigma=Sigma'*Sigma;  
ins=Sigma; 
i=[nalpha 0 0 0];  
...
stx.X=X; stx.Z=Z; stx.G=GG; stx.W=W; stx.T=T; stx.H=HH;
stx.ins=ins; stx.i=i;
...
stx.hb=hb; stx.Mb=Mb; stx.A=A; stx.P=P; stx.B=Bs; stx.D=Ds; stx.m1=m1;
stx.kro=str.kro;
