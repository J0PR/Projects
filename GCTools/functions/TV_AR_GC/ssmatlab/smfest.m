...
arp=models.arp; cycle=models.cycle;
if (arp + cycle) > 0
 ...
 if cycle > 0
  stord=models.stord;  xp=zeros(1,max(stord));  xp(stord)=x;
  xrho=xp(7);
  if (xrho < 0.) | (xrho > 1.)
   xp(7)=exp(xrho)/(1.+exp(xrho));
   x=xp(stord); xv=x(pvar); 
  end
  xlambda=xp(8); xl1=models.xl1; xl2=models.xl2;
  if (xlambda < xl1) | (xlambda > xl2)
   xp(8)=(xl1+xl2*exp(xlambda))/(1.+exp(xlambda));
   x=xp(stord); xv=x(pvar); 
  end
 end
end

[F,e,g,M,Pevf] = smfun(xv,y,s,pfix,pvar,xf,chb,models);
