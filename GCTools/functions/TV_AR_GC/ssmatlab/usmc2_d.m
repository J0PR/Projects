...
clear
% load data. 
yy=load('data\melanoma.dat'); x=[];
y=vec(yy'); y=y(1:end-3);
yor=y;
tname='melanoma';
lam=1;
if lam == 0
...
comp.freq=freq;
bg_year=1936; bg_per=1; datei = cal(bg_year,bg_per,freq); 
comp.datei=datei;
...
cycle=Xt(:,1); trend=X*g;
%forecast of cycle and trend
cyclep=alpr(1,:)'; trendp=Xp*g;
