...
aic=dn*(log(2*pi)+double(1)) + dn*log(Ff/dn) + 2*double(nr+nreg);
bic=dn*(log(2*pi)+double(1)) + dn*log(Ff/dn) + log(dn)*double(nr+nreg);

ne=length(e);                                    %residual sample length
ve=var(e,1); stde=sqrt(ve);                      %residual variance and std. dev.
%the following line added 21-11-2009
mser=Ss/(dn-nr-nreg); stder=sqrt(ve);            %mse and standard error of resid
...
%the following line added 21-11-2009
ec=e;
ic=1; [c0,cv,r]=autcov(ec,lag,ic);               %autocorrelations
[fi,pc]=durlev(c0,cv);                           %partial autocorrelations
orders = 1:lag;
[qstat,pval,df,sea] = lbs(ne,orders,r,nr);
sep=ones(lag,1)/sqrt(ne);
rstd=sqrt(ve/ne); rtval=me/rstd;               %standard deviation and t-value of mean
maxe=max(e); mde=median(e); mine=min(e);       %median, minimum and maximum
[skew,kurt,sk,ss] =  skewkur(e,me,ve,0,ne);    %skweness and kurtosis
dw = durwat(e,0,ne,Ss);                        %Durbin-Watson
tsk=skew/ss; tkr=(kurt-3)/sk; bst=tsk^2+tkr^2; %normality test
sqe=e.^2; 
h=round(ne/3);  H=sum(sqe(ne-h+1:ne))/...
...
pf=fdis_cdf(H,h,h); if (pf < .5), pH=2*pf; else  pH=2*(1-pf); end
ddw=4.d0/double(ne); tdw=(dw-2.d0)/sqrt(ddw);
[Result,Ccum] = cumnor(tdw); if (tdw < 0), ptdw=2*Result; else  ptdw=2*Ccum; end
[n0,n1,nr,Tval] = runcom(e,ne,mde);
% if ne <= 100                                     %compute number of bins for the histrogram
%  nbin=13;
% elseif ne <= 250
%  nbin=15;
% else
%  nbin=17;
% end
% [no,xo] = hist1(e,nbin);                       %residual histogram
[bin,cutpnt,otlrt0,otlr] = hist2(e,mde);
no=bin; xo=cutpnt; 
...
infr.e=e; infr.ne=ne; infr.ve=ve; infr.stde=stde; infr.sconp= sconp; infr.conp=conp;
infr.orders=orders; infr.r=r; infr.pc=pc; infr.qstat=qstat; infr.pval=pval; 
infr.df=df; infr.sea=sea; infr.sep=sep; infr.no=no; infr.xo=xo; infr.hot0=otlrt0; infr.ho=otlr;
infr.me=me; infr.rstd=rstd; infr.rtval=rtval; infr.maxe=maxe; infr.mde=mde;
infr.mine=mine; infr.skew=skew; infr.kurt=kurt; infr.dw=dw; infr.bst=bst; 
infr.pnt=pnt; infr.tsk=tsk; infr.tkr=tkr; infr.dw=dw; infr.tdw=tdw; infr.ptdw=ptdw; 
infr.n0=n0; infr.n1=n1; infr.nr=nr; infr.Tval=Tval; infr.rs=rs; infr.pcs=pcs; 
infr.qstats=qstats; infr.pvals=pvals; infr.dfs=dfs; infr.seas=seas; infr.h=h;
infr.H=H; infr.pH=pH; infr.aic=aic; infr.bic=bic; infr.mser=mser; infr.stder=stder;
