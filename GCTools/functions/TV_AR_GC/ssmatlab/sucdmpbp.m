...
% and a cycle by means of the application of a band-pass filter. See "The 
% Use of Butterworth Filters for Trend and Cycle Estimation in Economic 
% Time Series", Gómez, V. (2001), Journal of Business and Economic 
...
%---------------------------------------------------
% USAGE: [X,Z,G,W,T,H,ins,ii,strc,ferror] = sucdmpbp(comp,compf,y,Y,stra,npr)
% Inputs:     
...
if ~isfield(comp,'ptnum')
    disp('field ptnum must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 ptnum=comp.ptnum;
...
if ~isfield(comp,'ptden')
    disp('field ptden must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 ptden=comp.ptden;
...
if ~isfield(comp,'ptnur')
    disp('field ptnur must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 ptnur=comp.ptnur;
...
if ~isfield(comp,'ptvar')
    disp('field ptvar must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 ptvar=comp.ptvar;
...
if ~isfield(comp,'stnum')
    disp('field stnum must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 stnum=comp.stnum;
...
if ~isfield(comp,'stden')
    disp('field stden must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 stden=comp.stden;
...
if ~isfield(comp,'stnur')
    disp('field stnur must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 stnur=comp.stnur;
...
if ~isfield(comp,'stvar')
    disp('field stvar must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 stvar=comp.stvar;
...
if ~isfield(comp,'rt')
    disp('field rt must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 rt=comp.rt;
...
if ~isfield(comp,'rtvar')
    disp('field rtvar must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 rtvar=comp.rtvar;
...
if ~isfield(comp,'itvar')
    disp('field itvar must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 itvar=comp.itvar;
...
if ~isfield(comp,'sigmaa')
    disp('field sigmaa must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 sigmaa=comp.sigmaa;
...
if ~isfield(comp,'phi')
    disp('field phi must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 phi=comp.phi;
...
if ~isfield(compf,'num')
    disp('field num must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 num=compf.num;
...
if ~isfield(compf,'den')
    disp('field den must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 den=compf.den;
...
if ~isfield(compf,'Di')
    disp('field Di must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 Di=compf.Di;
...
if ~isfield(compf,'Alpha')
    disp('field Alpha must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 Alpha=compf.Alpha;
...
if ~isfield(compf,'sa')
    disp('field sa must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 sa=compf.sa;
...
%     disp('field Thetac must be present in structure comp in sucdmpbp.m')
%     ferror=1;
%     return
% else
%  Thetac=comp.Thetac;
...
if ~isfield(compf,'Lambda')
    disp('field Lambda must be present in structure comp in sucdmpbp.m')
    ferror=1;
    return
else
 Lambda=compf.Lambda;
...
np=length(ptnum); dp=length(ptden);
if np ~= dp
 disp('ptnum and ptden should have the same length in sucdmpbp')
 ferror=2;
 return
end
ns=length(stnum); ds=length(stden);
if ns ~= ds
 disp('stnum and stden should have the same length in sucdmpbp')
 ferror=2;
 return
end
...
% function cascade
%
% model for sp_t: phisp(z) sp_t = thsp(z) a_spt, where phisp(z) =
% ptden(z) den(z) and thsp(z) = ptnum(z)Alpha(z). Implemented 
% as the cascade [Alpha(z)/den(z)][ptnum(z)/ptden(z)]. There is no
% cancellation of terms.
sigmap=sqrt(ptvar)*(sigmaa)*(Lambda/sa);
[Tsp,Hsp,Zsp,ferror]=cascade(den,Alpha,ptden,ptnum,sigmap); 
% model for c_t: phic(z) c_t = thc(z) a_ct, where phic(z) =
% phi(z)den(z) and thc(z) = ptnum(z)(1-z)^(Di-ptnur)(1+z)^Di. Implemented 
% if p<=Di as the cascade [Alphac(z)/den(z)][(1+z)^p/phi(z)], where
% Alphac(z)=ptnum(z)*(1-z)^(Di-ptnur)(1+z)^(Di-p) and p is the degree of 
% phi. If p<=2*Di-ptnur, implemented as the cascade
% [Alphac(z)/den(z)][(1+z)^Di(1-z)^(p-Di)/phi(z)], where 
% Alphac(z)=ptnum(z)(1-z)^(2*Di-ptnur-p).
% The factor (1 - z)^ptnur is cancelled if Di >= ptnur because the 
% filter numerator is (1 - z)^Di(1 + z)^Di. If Di < ptnur, the factor
% (1-z)^Di is cancelled.
Alphac=num; mptndi=min(ptnur,Di);
for i=1:mptndi
...
if p == 0
 ...
 elseif p <= 2*Di - ptnur
  ...
  den=conv(den,phi);  phi=[0. 1.]; S=[0. 1];
 end
end
Alphac=conv(Alphac,ptnum);

sigmac=sqrt(ptvar)*(sigmaa)*(1/sa);  
[Tc,Hc,Zc,ferror]=cascade(den,Alphac,phi,S,sigmac); 

% matrix T
[nsp,junk]=size(Tsp); [nc,junk]=size(Tc);
np=nsp+nc;
nr=length(rt); nrm1=nr-1;
nalpha=np+ns+nr;
nppns=np+ns;
T=zeros(nalpha);
T(1:nsp,1:nsp)=Tsp; T(nsp+1:nsp+nc,nsp+1:nsp+nc)=Tc;
Ts=zeros(ns); nsm1=ns-1;
Ts(1:nsm1,2:end)=eye(nsm1); Ts(ns,2:ns)=-stden(1:end-1);
Tr=zeros(nr);
if nr > 1
...
H=zeros(nalpha,nch);
H(1:nsp,1)=Hsp; H(nsp+1:nsp+nc,2)=Hc;
...
% matrix G
G=zeros(1,nch); G(nch)=sqrt(itvar)*(sigmaa);
% matrices Z, W and X
Z=zeros(1,nalpha); Z(1)=1.; Z(nsp+1)=1; Z(np+1)=1.;
if (nr > 0)
 Z(nppns+1)=1.;
end
W=[];  
...
strc.X=X; strc.Z=Z; strc.G=G; strc.W=W; strc.T=T;
strc.H=H; strc.ins=ins; strc.i=ii;
