...
if nlag == 0
 resid=yc;   sigmar=cov(resid,1); 
 return
end


%augment resid, x and y with zeros
npl=nobs+nlag; nlp1=nlag+1;
resida=zeros(npl,neqs); ya=zeros(npl,neqs); 
ya(nlp1:npl,:)=yc;
if (nx > 0)
 xa=zeros(npl,nx); xa(nlp1:npl,:)=x;
end

thetast=str.thetast; phist=str.phist; gammast=str.gammast;
Th=[]; Ph=[]; Gm=[];
for i=2:nlp1
 Th=[Th thetast(:,:,i)]; Ph=[Ph phist(:,:,i)]; 
 if (nx > 0)
  Gm=[Gm gammast(:,:,i)];
 end
end
