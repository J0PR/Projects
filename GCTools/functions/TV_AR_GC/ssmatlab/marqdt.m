...
while conv == 0
 ...
 flag=0;
 while flag == 0
  if nu > 0,   wk=[fjac; sqrt(nu)*eye(n)];  else   wk=fjac;     end
  [Q,RR]=qr(wk);  R=RR(1:n,1:n);
  if rank(R) == n,   flag=1;  else
   if nu == 0,    nu=0.01;   else    nu=4*nu;   end
  end
 end
 delta = R\(R'\(-g));
 xd=x+delta';
%  xd
 if infoz.tr == 0  
  [ffd,xd]=feval(infoz.f,xd,varargin{:});  conf=conf+1; 
 else
  ffd=feval(infoz.f,xd,varargin{:});  conf=conf+1; 
 end
 rs=ffd'*ffd; 
 rr=abs(rs0-rs)/(1+abs(rs0));
 rp=max(abs(x-xd)./(ones(size(x))+abs(x)));
 if infoz.prt == 2
  fprintf(1,' it= %2i',iter); fprintf(1,' rs= %12.8f',rs); 
  fprintf(1,'    rr= %12.8f',rr); fprintf(1,'    rp= %12.8f',rp);
  fprintf(1,'    nu= %12.8f\n',nu);
 end
 ...
 %  [fjac,g] = fdjac2(infoz,x,ff,varargin{:}); 
  ...
  if s < 0.25 
   if (nu == 0), nu=.001; else nu=4*nu; end
  elseif s > .75
   nu=nu/2;
  end
  if s > 0 
...
if infoz.prt > 0
 fprintf(1,'\n rs= %12.8f',rs); fprintf(1,'    rr= %12.8f',rr);
 fprintf(1,'    rp= %12.8f',rp); fprintf(1,'    nu= %12.8f\n',nu)
end
