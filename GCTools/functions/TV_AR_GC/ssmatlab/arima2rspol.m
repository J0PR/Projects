...
if ~isempty(phi)
    ...
    rphi=roots(phi);
    if (any(rphi > 0)) || (~isreal(rphi))
        j = (rphi>0) | (imag(rphi) ~= 0);
        prphi=rphi(j);
        lprphi=length(prphi);
        phir=1.;
        for i=1:lprphi
...
if ~isempty(Phi)
    Phi=squeeze(Phi)';  [nP,mP,lP]=size(Phi);
    if lP > 1
     Phix=zeros(1,s+1); Phix(1)=1.; Phix(end)=Phi(end);   
