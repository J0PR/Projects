...
ins=str.ins; i=str.i; X=str.X; W=str.W; Z=str.Z; 
s=str.freq; pfix=str.pfix; pvar=str.pvar; stord=str.stord;
conc=str.conc; trend=str.trend; slope=str.slope; seas=str.seas;
irreg=str.irreg; arp=str.arp;
...
if trend == 1
 if slope == 0
  T=1; H=xp(2);  
 elseif slope == 1
  T(1:2,1:2)=[1 1;0 1]; H(1,1)=xp(2); H(2,2)=xp(3); 
 elseif slope == -1
  T=1; H=xp(2);  
 end
elseif trend == 2
...
elseif seas == 2                              %trigonometric seasonality
 ...
 T(nT+sm1,nT+sm1)=-1;  H(nT+1:nT+sm1,nH+1:nH+sm1)=AA;
 if s == 4
...
elseif seas == 4                              %Butterworth tan seasonality
 [nT,junk]=size(T);	[junk,nH]=size(H);  
 T(nT+sm1,nT+sm1)=-1; T(nT+s,nT+s)=0;
 if s == 4
  T(nT+1:nT+2,nT+1:nT+2)=[0 1; -1 0];
