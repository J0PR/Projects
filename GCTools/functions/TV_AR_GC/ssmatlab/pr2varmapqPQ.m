...
if isempty(xv)
 ...
 if isempty(xf)
  ...
  x=zeros(size(xi));
  indvar=find(xi); indfix=find(~xi);
  x(indvar)=xv; x(indfix)=xf;
 end
end

%pass parameters to VARMA model
phi=str.phi; th=str.th; Phi=str.Phi; Th=str.Th; Lh=str.Lh;
[np,mp,pm1] = size(phi); p=pm1-1;
[nt,mt,qm1] = size(th);  q=qm1-1;
[nP,mP,Pm1] = size(Phi); P=Pm1-1;
[nT,mT,Qm1] = size(Th);  Q=Qm1-1;
nlh=length(Lh);
np2=np*np; j=0;
