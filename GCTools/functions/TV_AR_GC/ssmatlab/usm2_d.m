...
clear
% load data. 
btoz=load('data\btozone.dat'); [nb,junk]=size(btoz); nyb=nb-12;
yor=btoz(1:nyb,1); x=[];
tname='btozone'; fname='results\btozone.txt';
bg_year=1955; bg_per=1; freq=12; datei = cal(bg_year,bg_per,freq);
y=log(btoz(1:nyb,1)); lam=0;
Ya=btoz(:,2:4); 
ct=deltafil(Ya(:,2:3),0,1,0,0,freq); Ya=[Ya(:,1) ct];
Y=Ya(1:nb,:); nreg=3; 
