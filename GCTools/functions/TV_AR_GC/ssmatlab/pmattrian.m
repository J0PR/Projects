...
[n,m,p] = size(A); 
T=[]; U=[]; ierrpmatri = 0; %Initialization
...
M=A(:); nm=length(M);
normam=norm(M); tol=double(nm)*normam*eps(normam);   
...
dr=zeros(n,1); ds= zeros(m,1); %Initialization of degrees of rows and columns
