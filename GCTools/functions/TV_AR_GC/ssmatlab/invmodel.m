...
%
 ...
 str.noninv=1; ierror=0;
 
 
 % obtain the invertible model using the DARE
 stre.F=str.Fs; stre.H=str.Hs; stre.K=str.Ks; stre.Sigma=str.sigmar2;
 stre.kro=str.kro; 
 stre.npar=str.npar; stre.s=str.s; stre.m=str.m;

 
 stre.phi=str.phis;  stre.theta=str.thetas; stre.gamma=str.gammas;
...
if (ferror > 0)
 ...
 % change estimates
 str.sigmar2=stre.Sigmab; str.thetas=stre.thetab;  
 str.Ks=stre.Kb;
 
 
 stre.phi=str.phis; stre.theta=str.thetas; stre.gamma=str.gammas; stre.s=str.s; 
 stre.m=str.m; stre.kro=str.kro;  
 stre = vecparwr(stre); %fill in vgam with new model parameters  
 neqs=str.s;   
 vgamsc=stre.vgam; vgams=str.vgams; 
 ...
 bind=str.bind;
 [nbind,mbind]=size(bind); beta=zeros(size(bind));
 for i=1:nbind
  beta(i)=vgams(bind(i));
 end

 str.vgams=vgams; str.beta=beta;
