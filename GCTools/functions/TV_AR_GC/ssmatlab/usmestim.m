...
%initial parameters
xv=str.xv; xf=str.xf; pfix=str.pfix; pvar=str.pvar; X=str.X;
% this line added 12-2011
W=str.W;
% x0=str.x; 
nr=length(pvar); s=str.freq; 


%Levenberg-Marquardt method
f='smfest'; if ((str.arp > 0) || (str.cycle > 0)), tr=0; else tr=1; end
mvx=1; chb=0; prt=2;
tolf=1e-4;  nu0=.01; jac=1; 
...
maxabsxvidx=0; maxabsxv=-1.d10; stord=str.stord;
for i=1:nr
 if (stord(pvar(i)) <= 6)
  absxv=abs(xvp(i));
  if (absxv > 1.) && (absxv > maxabsxv)
   maxabsxv=absxv;  maxabsxvidx=i;  
  end
 end
end
if (maxabsxvidx > 0)        %change of concentration parameter
 oldconc=str.conc; str.conc=stord(pvar(maxabsxvidx)); 
 stord(pvar(maxabsxvidx))=oldconc;   
 oldstordc=stord; nstord=sort(stord); str.stord=nstord; 
...
%   oldx, pause
 ...
 oldpvar=pvar; oldpfix=pfix;
 npvar=zeros(size(pvar)); npfix=zeros(size(pfix)); nf=length(pfix);
 ...
 [npvar,ipvar]=sort(npvar); [npfix,ipfix]=sort(npfix);  
 % a new variance has been concentrated out, but the initial conditions
 % are the same.
 xv=xv(ipvar); xf=xf(ipfix); pvar=npvar; pfix=npfix;
 str.pvar=npvar; str.pfix=npfix;  
 str.xv=xv; str.xf=xf; str.x(npvar)=xv; str.x(npfix)=xf; 
 [X,Z,G,W,T,H,ins,ii,ferror] = pr2usm(xv,xf,str);  
 str.X=X; str.Z=Z; str.G=G; str.W=W; str.T=T; str.H=H; str.i=ii;
%   str,pause
end
...
[beta,tv,se]=btval([],[res2t V]);% tv contains the t-values
...
%
[F,e,h,M,Pevf,A,P]=smfun(xvf,y,s,pfix,pvar,xf,chb,str);
...
result.xvf= xvf; result.xf = xf; 
result.e = e; result.Ss=Ss; result.Ff=Ff;
result.sigma2c = sigma2c; 
result.Pevf = Pevf; SPevf=sqrt(diag(Pevf));
result.SPevf=SPevf;
result.tv = tv'; result.se=se';
result.F = F; result.h = h; result.M = M; 
result.A=A; result.P=P;
% this line changed 12-2011
if ~isempty(X) || ~isempty(W)
 %t-values of regression parameters
 M=M*sigma2c; ser=sqrt(diag(M));
 tvr=h./ser; result.tvr=tvr; result.ser=ser;
else
 result.tvr=[]; result.ser=[];
end
