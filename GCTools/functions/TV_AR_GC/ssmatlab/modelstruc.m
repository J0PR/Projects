function models = modelstruc(xx,y,Y,pfix,pvar,xf,modescr,datei,npr)
%************************************************************************
...
s=datei.freq; conc=modescr.conc; stord=modescr.stord;
...
C=[]; A=[];                     % matrices for initial conditions
W=[];  Z=[]; T=[]; H=[]; G=[];  % system matrices
...
elseif trend == 1
 ...
 elseif slope == 1
  	Z(1)=1;	T(1:2,1:2)=[1 1;0 1]; H(1,1)=xp(2); H(2,2)=xp(3); 
   A(1:2,1:2)=eye(2); Z(2)=0;
 elseif slope == -1
  X=[constant(ny+npr,1,1,0,0,0,s) X];
 	Z(1)=1; T=1; H=xp(2); A(1,1)=1;
 end
elseif trend == 2
 if slope == 0
  ...
  T(2,2)=0; A(1,1)=1; 
 elseif slope == 1
 	den=abs(xp(3))+abs(xp(2)); th=(abs(xp(3))-abs(xp(2)))/den; 
  Z(1)=1;	Z(3)=1;	T(1:2,1:2)=[0 1;-1 2]; T(3,3)=0;
	 H(1,1)=(3+th)*den; H(2,1)=(5+3*th)*den; H(3,2)=den; 
  A(1:2,1:2)=eye(2); C(3,3)=den^2; 
 elseif slope == -1
  X=[constant(ny+npr,1,1,0,0,0,s) X];
 	Z(1)=1;	Z(2)=1;	T(1,1)=1;	H(1,1)=2*xp(2); H(2,2)=xp(2);
  T(2,2)=0; A(1,1)=1;
...
elseif seas == 1                              %stochastic dummy seasonality
 [nT,junk]=size(T);	[junk,nH]=size(H); Z(nT+1)=1; [junk,nA]=size(A);
 T(nT+1,nT+1:nT+sm1)=-ones(1,sm1); Z(nT+sm1)=0;
	T(nT+2:nT+sm1,nT+1:nT+sm1-1)=eye(sm1-1);	
 H(nT+1,nH+1)=xp(4); H(nT+sm1,nH+1)=0;
 A(nT+1:nT+sm1,nA+1:nA+sm1)=eye(sm1);          
elseif seas == 2                              %trigonometric seasonality
 [nT,junk]=size(T);	[junk,nH]=size(H); Z(nT+1)=1; [junk,nA]=size(A);
 AA=zeros(sm1); for i=1:sm1, AA(i,i)=xp(4); end
 T(nT+sm1,nT+sm1)=-1; Z(nT+sm1)=1;
 H(nT+1:nT+sm1,nH+1:nH+sm1)=AA;
 if s == 4
  T(nT+1:nT+2,nT+1:nT+2)=[0 1; -1 0]; 
 elseif s == 12
  sqrt3d2=sqrt(3)/2;
  Z(nT+3)=1; Z(nT+5)=1; Z(nT+7)=1; Z(nT+9)=1; 
...
elseif seas == 4                              %Butterworth tan seasonality
 [nT,junk]=size(T);	Z(nT+1)=1;	[junk,nH]=size(H); [junk,nA]=size(A);
 T(nT+sm1,nT+sm1)=-1; Z(nT+sm1)=1; Z(nT+s)=1; T(nT+s,nT+s)=0;
 if s == 4
  T(nT+1:nT+2,nT+1:nT+2)=[0 1; -1 0];
  H(nT+2,nH+1)=-2*xp(4); H(nT+3,nH+2)=-2*xp(4); 
 ...
 elseif s==12
  ...
  H(nT+1:nT+2,nH+1)=[sqrt3*xp(4); xp(4)]; H(nT+3:nT+4,nH+2)=[xp(4); -xp(4)];
  H(nT+6,nH+3)=-2*xp(4); H(nT+7:nT+8,nH+4)=[-xp(4); -xp(4)]; 
  H(nT+9:nT+10,nH+5)=[-sqrt3*xp(4); xp(4)]; H(nT+11,nH+6)=-2*xp(4); 
  H(nT+12,nH+7)=sqrt(6)*xp(4);
 end
 A(nT+1:nT+sm1,nA+1:nA+sm1)=eye(sm1); A(nT+s,nA+sm1)=0;       
 C(nT+s,nT+s)=floor(s/2)*xp(4)^2;
end

if cycle == 1    %structural model cycle (Harvey)
 [nT,junk]=size(T);	[junk,nH]=size(H); Z(nT+1)=1;
 xp6=xp(6);
 [junk,nA]=size(A); sc=xp6*sqrt((1. - xp(7)^2));
...
[mH,nH]=size(H);
if irreg > 0
 G(1,nH+1)=xp(1); H(1,nH+1)=0;
else
 G=zeros(1,nH);
end

if arp > 0
 [nT,junk]=size(T); [junk,nH]=size(H); [junk,nA]=size(A);
 if arp > 1
 ...
 Z(end+arp)=1; H(nT+arp,nH+1)=xp(5); G(end+1)=0;
...
[nC,mC]=size(C);   [nalpha,junk]=size(T);
if (nC > 0) & (nC < nalpha)
 C(nalpha,nalpha)=0.; nC=nalpha;
end
[junk,nA]=size(A);  
ins=[C A]; i=[nC 0 0 nA];
...
% ins
% i
iT=find(T); [nT,mT]=size(T);
if (iT <= .75*nT*mT), T=sparse(T); end
iG=find(G); [nG,mG]=size(G);
if (iG <= .75*nG*mG), G=sparse(G); end
iH=find(H); [nH,mH]=size(H);
if (iH <= .75*nH*mH), H=sparse(H); end

models.Z=Z; models.T=T; models.X=X; models.W=W; models.G=G;
models.H=H; models.ins=ins; models.i=i;
models.stord=stord; models.conc=conc; models.trend=trend;
models.slope=slope; models.seas=seas; 
models.cycle=cycle; models.xl1=xl1; models.xl2=xl2;
models.irreg=irreg; models.arp=arp;
