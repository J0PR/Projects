...
%
phis=str.phi; thetas=str.theta; gammas=str.gamma; s=str.s;
kro=str.kro; vgams=str.vgams; nx=str.m; 

vgamtv=str.vgamtv;
phitv=NaN(size(phis)); thetatv=NaN(size(thetas)); gammatv=NaN(size(gammas)); 

neqs2=s*s;  nlag=max(kro); 
for i=2:nlag+1
 A=[]; Atv=[];  
 for j=1:s
...
%  thetatv(:,:,i)=Atv;
% end
phis(:,:,1)=thetas(:,:,1); phitv(:,:,1)=thetatv(:,:,1);
ii=ii+i*neqs2;
if (nx > 0)
 for i=1:nlag+1
 A=[]; Atv=[];
...
if nx == 0
 ...
 musers=Phi\mu; str.musers=musers;  % mean value obtained from the constant
end

str.phis=phis; str.phitv=phitv;
str.thetas=thetas; str.thetatv=thetatv;
str.gammas=gammas; str.gammatv=gammatv;
str.mu=mu; str.mutv=mutv;
