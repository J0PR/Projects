...
arp=modescr.arp; cycle=modescr.cycle;
pfix=modescr.pfix; pvar=modescr.pvar;
if modescr.trend == -1, flevel=1; else  flevel=0; end
if modescr.slope == -1, fslope=1; else  fslope=0; end
if modescr.seas == -1, fseas=1; else  fseas=0; end
xx=modescr.x;
xx(pvar)=result.xvf;                      %estimated parameters
tt=result.tv; se=result.se;
hb=result.h; tb=result.tvr; seb=result.ser;
sconp=sqrt(result.sigma2c); 

%print estimation results
inft = minft(fid,1,9,3,1);
dateib=datei; inftb=inft; fnameb=tname;
prtser(fid,fnameb,yor,y,ny,dateib,inftb,lam);

fprintf(fid,'Estimation results:\n');
stord=modescr.stord; conc=modescr.conc;  
arpc=arp;
if cycle > 0
 arpc=arp+2;
end
nst=length(stord)-arpc;
if nst > 0, xx(1:end-arpc)=xx(1:end-arpc)*sconp; end
sse=zeros(size(xx)); sse(pfix)=NaN; sse(pvar)=se;
%this line added 12-2011
tte=zeros(size(xx)); tte(pfix)=NaN; tte(pvar)=tt; 
%this line changed 12-2011
xx=[sconp xx]; sse=[NaN sse]; tte=[NaN tte];
z=[xx' sse' tte'];
clear in
in.cnames = strvcat('  Estimate','Std. Error','   T-ratio');
in.rnames = ['Parameter   '];
rnamess=strvcat('Sigma irreg.','Sigma level ','Sigma slope ',...
         'Sigma seaso.','Sigma autor.','Sigma cycle');
in.rnames=[in.rnames; rnamess(conc,:)];
if nst > 0  
 for i=1:nst, in.rnames=[in.rnames; rnamess(stord(i),:)]; end 
end
if cycle > 0
 in.rnames=strvcat(in.rnames,'Cycle rho ','Cycle freq.'); 
end
if arp > 0
 rnamesa=[];
 for i=1:arp, rnamesa=strvcat(rnamesa,['Ar(' num2str(i) ')       ']); end
 for i=1:arp, in.rnames=[in.rnames; rnamesa(i,:)]; end
end
...
%*************************************************************************
% These lines added 01.2012 by Martyna Marczak
...
%  seb=sqrt(diag(Mb*conp)); tb=hb./seb;             %standard errors and t-values    
 ...
 clear in                                                                          
 in.cnames = strvcat('  Estimate','Std. Error','   T-ratio');                      
 rnames = strvcat('Parameter');
 if flevel == 1, rnames=strvcat(rnames,['level']); end
 if fslope == 1, rnames=strvcat(rnames,['slope']); end
 if fseas == 1, for i=1:s-1, rnames=strvcat(rnames,['seas' num2str(i)]);...
    end,end;
 if nreg > 0
  for i=1:nreg, rnames=strvcat(rnames,['reg' num2str(i)]); end        
 end
 in.rnames=rnames; in.fmt=strvcat('%12.5f','%12.5f','%8.2f');                      
 in.fid=fid; mprint(Mbeta,in);                                                     
 fprintf(fid,'\n');                                                                
end                                                                                 
