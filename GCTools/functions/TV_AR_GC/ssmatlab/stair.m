...
n=nb; m=mb; %k=zeros(1,n); 
[q,r,e]=qr(b); b1= r*e'; a1=q'*a*q; 
u=q; k(1)=rank(r);
k1=1; k2=k(1)+1;
...
for i=2:n
 ...
 a1=q'*a1*q; a1(k2:n,k1:k2-1)=r*e'; u=u*q; rk=rank(r,tol); 
 if (k2+rk <= n), a1(k2+rk:n,k1:k2-1)=zeros(n-k2-rk+1,k2-k1); end
 if rk == 0, break, end
 k(i)=rk; k1=k2; k2=k2+rk;
 if k2 > n, break, end
end

%obtain controllability indices in descending order
C=zeros(1,m); [nk,mk]=size(k);
for i=1:mk
 for j=1:k(i)
  C(j)=C(j)+1;
 end
...
if nargin == 4
 ...
 if kmax > 1
  for i=kmax:-1:2
   k1=k1-k(i-1);
   if k2 > k1+1
    [q,r]=qr(pert(a1(k2:k3,k1:k2-1),adj));  q=pert(q,adj)';  
 ...
 [q,r]=qr(pert(b1(k2:k3,:),adj));  q=pert(q,adj)';  
 v=v*q; b1=b1*q;
