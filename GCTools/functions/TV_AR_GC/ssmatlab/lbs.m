...
if (nargin ~= 4)    error('Wrong number of arguments to lbs'); end;  
np=length(p);
qstat=zeros(np,1); pval=zeros(np,1); df=zeros(np,1); sea=zeros(np,1);
sea(1)=1/sqrt(ne); s=0;                 %standard errors
for i=1:np-1
 s=s+r(i)*r(i); sea(i+1)=sqrt((1+2*s)/ne);
end
for i=1:np; 
