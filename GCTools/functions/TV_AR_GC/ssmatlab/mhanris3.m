...
if nlag == 0    %the only parameter is the mean
 str.resid3=str.resid2; str.sigmar3=str.sigmar2; 
 return
end
...
if ~isfield(str,'resid23') | (isfield(str,'resid23') & isempty(str.resid23))
 ...
 resid2=str.resid23; sigmar2=str.sigmar23;
end
...
%
str.beta3=[]; str.tv3=[]; str.vgams3=[]; str.vgamtv3=[];
str.mus3=[]; str.phis3=[]; str.thetas3=[]; str.gammas3=[];
str.phitv3=[]; str.thetatv3=[]; str.gammatv3=[]; str.mutv3=[];  


resid=resid2; sigmar=sigmar2;  
% parameters passed from second step
phis=str.phis; thetast=str.thetast; phist=str.phist; gammast=str.gammast;
beta=str.beta;
...
A=[-ylag -vp reslag xlag ones(nobse,1)]; [na,ma]=size(A);
xmat=zeros(na*neqs,ma*neqs);
for i=1:na
 for j=1:ma
  aa=repmat(A(i,j),1,neqs);dd=diag(aa);
...
thzrt=[]; [nzr,mzr]=size(zr); Phi0=phis(:,:,1);
for i=1:nobse
...
lres2=L\resid(nlag+1:end,:)'; res2t=vec(lres2); % standardized residuals
...
str3 = param2sse(str3);
%store results
str.phis3=str3.phis; str.thetas3=str3.thetas; str.gammas3=str3.gammas;
str.phitv3=str3.phitv; str.thetatv3=str3.thetatv; str.gammatv3=str3.gammatv;
str.mus3=str3.mu; str.mutv3=str3.mutv;
str.phist3=str3.phist; str.thetast3=str3.thetast; str.gammast3=str3.gammast;
str.Fs3=str3.Fs; str.Ks3=str3.Ks; str.Bs3=str3.Bs; str.Ds3=str3.Ds;
str.Hs3=str3.Hs;
...
resid3v=res2t-V*beta3; resid3=zeros(nobse,neqs);        
for i=1:nobse
 resid3(i,:)=(L*resid3v((i-1)*neqs+1:i*neqs))';
end
sigmar3=cov(resid3,1); str.resid3=resid3; str.sigmar3=sigmar3;
