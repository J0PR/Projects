...
%**************************************************************************
 ...
 my=min(abs(yor)); if my <= 1d-9, inft.scale=0; end %do not scale if numbers are too small
 tabla(yor,datei,inft); fprintf(fid,'\n');
 if lam == 0                                     %print series in logs
  fprintf(fid,'Logs of the series:\n');
  fprintf(fid,'Number of observations:%5i\n',ny);
  my=min(abs(y)); if my <= 1d-9, inft.scale=0; end %do not scale if numbers are too small
  tabla(y,datei,inft); fprintf(fid,'\n');
 end
