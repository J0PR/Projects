...
% ESTVARAMXKRO for the Hannan-Rissanen method or function MCONESTIM for the 
...
%         Y: matrix for regression variables. If empty, the variables are
%            centered and the mean is not estimated. 
...
if iar > 1
 fprintf(1,'initial model nonstationary in mexactestim, iar = %2d\n',iar);
 str = sta3r(str); 
end
ima=chkstainv(str.Fs3-str.Ks3*str.Hs3);   %if ima >1, the model is not invertible
if ima > 1
 fprintf(1,'inital model noninvertible in mexactestim, ima = %2d\n',ima);
 str = inv3r(str);
end
...
[R,p] = chol(sigmar); L=R';
if (p > 0)
 disp('covariance matrix of residuals singular in mexactestim')
 ferror=1; return
end
...
f='exactmedfvc'; tr=0; mvx=1; %parameters for Levenberg-Marquardt method:
tolf=1e-4;                 %f  :   a function to evaluate the vector ff of individual
maxit=100; nu0=.01;        %       functions such that ff'*ff is minimized  
...
chb=0;                     
[xvf,J,ff,g,iter,conf] = marqdt(infm,xv,y,x,str,Y,chb);
...
[ff,xvft,e,f,str,stx]=exactmedfvc(xvf,y,x,str,Y,chb); 
hb=stx.hb; Mb=stx.Mb;
sigma2c=e'*e/double(ny*my);             %concentrated parameter estimate
str.sigma2c=sigma2c;
bind=str.bind;[nbind,mbind]=size(bind); %nbind includes the mean parameters
nparma=nbind-my;                   %number of parameters in ar and ma parts
L=zeros(my); 
l=0; betam=[1.  xvf(nparma+1:end)];
for i=1:my
 cont=my-i+1; ind=l+1:l+cont;
 L(i:end,i)=betam(ind); l=l+cont;
end
...
lLh=length(Lh); lbeta=length(beta);
if lLh > 1
 str.Lh=beta(lbeta-length(Lh)+2:lbeta); str.Lhtv3=tv3(end-length(Lh)+2:end); 
end

%use an auxiliary structure to compute the state space and VARMAX
%echelon forms corresponding to the exact m.l. estimation
str3=str; str3.vgams=str.vgams3; str3.vgamtv=str.vgamtv3;
str3 = param2sse(str3);
%store results
str.phisexct=str3.phis; str.thetasexct=str3.thetas; str.gammasexct=str3.gammas;
str.phitvexct=str3.phitv; str.thetatvexct=str3.thetatv; str.gammatvexct=str3.gammatv;
str.musexct=hb; str.mutvexct=hb./sqrt(diag(Mb));
str.phistexct=str3.phist; str.thetastexct=str3.thetast; str.gammastexct=str3.gammast;
str.Fsexct=str3.Fs; str.Ksexct=str3.Ks; str.Bsexct=str3.Bs; str.Dsexct=str3.Ds;
str.Hsexct=str3.Hs;