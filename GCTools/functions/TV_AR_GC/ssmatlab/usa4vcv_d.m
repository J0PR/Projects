...
figure
subplot(2,2,1);
plot(usa_2003q1(1:n,1),y(:,1)); title('Log of GDP');

subplot(2,2,2);
plot(usa_2003q1(1:n,1),y(:,2)); title('Inflation');

subplot(2,2,3);
plot(usa_2003q1(1:n,1),y(:,3)); title('Unemployment rate');

subplot(2,2,4);
plot(usa_2003q1(1:n,1),y(:,4)); title('G. P. Investment');

disp('to delete all figures, call function closefig')
...
info.f='usa4vcvf'; info.tr=1; 
info.tolf=1e-4; info.tolx=sqrt(info.tolf); info.max=300; info.nu0=.01; info.prt=2;
tic
...
tt=zeros(size(xx));
tt(pfix)=NaN;
tt(pvar)=xut./sqrt(abs(diag(SS)))'; disp(tt(pvar))
...
fid=fopen('results\usa4vcvtf.txt','w');
fprintf(fid,'%15.8f%15.8f%15.8f%15.8f\n',trendf(:,1:4)');
fclose(fid);
fid=fopen('results\usa4vcvstf.txt','w');   
fprintf(fid,'%15.8f%15.8f%15.8f%15.8f\n',strendf(:,1:4)'); 
fclose(fid);  

fid=fopen('results\usa4vcvcf.txt','w');
fprintf(fid,'%15.8f%15.8f%15.8f%15.8f\n',cicf(:,1:4)');
fclose(fid);
fid=fopen('results\usa4vcvscf.txt','w');   
fprintf(fid,'%15.8f%15.8f%15.8f%15.8f\n',scicf(:,1:4)'); 
fclose(fid);
...
fid=fopen('results\usa4vcvts.txt','w');
fprintf(fid,'%15.8f%15.8f%15.8f%15.8f\n',trends(:,1:4)');
fclose(fid);
fid=fopen('results\usa4vcvsts.txt','w');   
fprintf(fid,'%15.8f%15.8f%15.8f%15.8f\n',strends(:,1:4)'); 
fclose(fid);    

fid=fopen('results\usa4vcvcs.txt','w');
fprintf(fid,'%15.8f%15.8f%15.8f%15.8f\n',cics(:,1:4)');
fclose(fid);
fid=fopen('results\usa4vcvscs.txt','w');   
fprintf(fid,'%15.8f%15.8f%15.8f%15.8f\n',scics(:,1:4)'); 
fclose(fid);
