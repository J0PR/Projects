...
%          .fhead: flag for header and years
%             .wd: format width
...
% Example: datei(1900,1,6) and info.hd=0 will print only the data with six
...
fsmall = 0; fbig = 0;
if scale == 1
    eps=1e-10; ys=y+ones(size(y))*eps;   %scale data if necessary
    minly=min(log10(abs(ys))); if minly < 0, minly=-minly; else minly=0; end
    if minly > 1
     small = min(9,floor(minly)); fsmall = 1;
     y = y*10^small;
    end
    maxly=max(log10(abs(ys)));
    if maxly > 5
     big = min(9,floor(maxly)) - 3; fbig = 1;
     y = y*10^(-big);
    end
end
...
npi=datei.beg_per; npf=datef.period;  %initial and final periods
fwd=num2str(wd); fnd=num2str(nd);
...
if (ns > 1) & ((ns == 12) | (ns == 6) | (ns == 5) | (ns == 4) | (ns == 3) |...
  (ns == 2)) & (fh == 1) fmby=['%' fwd 's']; fprintf(fid,'%5s ','Year'); 
 for i=1:ns, fprintf(fid,fmby,strjust(yearc(i,:),'right'));  end
 fprintf(fid,'\n');
end
%first row
t=1:ns-(npi-1);                              %observation indices
if fh == 1, fprintf(fid,fmi,fy); elseif fh == 0, fprintf(fid,fmi,' '); end
if npi > 1
 nb=num2str(wd*(npi-1)); fmb=['%' nb 's'];   %format for the first blanks
 fprintf(fid,fmb,' '); fprintf(fid,fmd,y(t)); fprintf(fid,'\n');
else    
 fprintf(fid,fmd,y(t)); fprintf(fid,'\n');
end
%intermediate rows
if ny > 1
 for i=2:ny-1
  fy=fy+1;  t=(i-1)*ns-(npi-2):i*ns-(npi-1);
  if fh == 1, fprintf(fid,fmi,fy); elseif fh == 0, fprintf(fid,fmi,' '); end
  fprintf(fid,fmd,y(t)); fprintf(fid,'\n');
 end    
end
%last row 
if ny > 2
 fy=fy+1; t=(ny-1)*ns-(npi-2):(ny-1)*ns-npi+npf+1;
 if fh == 1, fprintf(fid,fmi,fy); elseif fh == 0, fprintf(fid,fmi,' '); end
 fprintf(fid,fmd,y(t)); fprintf(fid,'%s\n','');
end
%write note on scaling if necessary
if fsmall == 1,  fprintf(fid,' _________\n'); fprintf(fid,' Values should be multiplied by 1e-%i',small); end
if fbig == 1,  fprintf(fid,' _________\n'); fprintf(fid,' Values should be multiplied by 1e+%i',big); end
