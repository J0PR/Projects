function [c0s,cvs,rs,fis,pcs] = sacspacdif(y,tname,dr,ds,freq,lag,cw)
%*************************************************************************
%  This function outputs cv0s, cvs, rs, fis and pcs described below.
%  In addition, it plots the series before and after differencing, 
%  as well as the sample autocorrelations and the sample partial 
%  correlations of the differenced series.
%
%    INPUTS :
%         y : series
...
subplot(2,2,1)
plot(yor);
title(tname); axis('tight');
subplot(2,2,2)
plot(y);
dfname=['Differenced series: (' num2str(dr) ',' num2str(ds) ')'];
title(dfname); axis('tight');
yc=y-mean(y)*ones(size(y));                           %series must be mean-corrected  
ic=1; [c0s,cvs,rs]=autcov(yc,lag,ic);                 %autocorrelations
[fis,pcs]=durlev(c0s,cvs);                            %partial autocorrelations
subplot(2,2,3)
bar(rs); title('Sample autocorrelations');
n=length(y); t=1:lag;
hold on; sea=zeros(lag,1); sea(1)=1/sqrt(n); k=0;                 %standard errors
for i=1:lag-1
 k=k+rs(i)*rs(i); sea(i+1)=sqrt((1+2*k)/n);
end
plot(t,sea*cw); hold on; plot(t,-sea*cw); hold off;
subplot(2,2,4)
bar(pcs); title('Sample partial autocorrelations');
hold on
sep=ones(lag,1)*1/sqrt(n);                 %standard errors
plot(t,sep*cw); hold on; plot(t,-sep*cw); hold off;
