...
%
% This function estimates a VARMAX(p,p,p) model using the exact maximum 
% likelihood method. 
%
%
...
[nY,mY]=size(Y); nbeta=mY;

%check stationarity and invertibility. 
H=str.Z; F=str.T; G=str.H; J=str.G;
nxi=length(str.xi);nxv=sum(str.xi); nxf=nxi-nxv; 
xv=zeros(1,nxv); xf=zeros(1,nxf);  xi=str.xi;
%check stationarity
[U, T] = schur(F','complex');  
T = T';
...
%  ferror=1; return
 phi=enfstabpol(str.phi);  Phi=enfstabpol(str.Phi); 
 th=str.th; Th=str.Th; Sigma=str.Sigma; freq=str.freq;  
...
%  ferror=1; return
 th=enfstabpol(str.th);  Th=enfstabpol(str.Th);
 phi=str.phi; Phi=str.Phi; Sigma=str.Sigma; freq=str.freq;
 [str,ferror] = suvarmapqPQ(phi,th,Phi,Th,Sigma,freq);
 cont=0; contf=0;  
 for i=1:nxi
  if xi(i) == 1
...
%initial parameters
xv=str.xv; xf=str.xf; chb=0; 




f='lkevarmapqPQ'; tr=0; mvx=1; %parameters for Levenberg-Marquardt method:
...
Lh=str.Lh; nlhm1=length(Lh)-1;  
L=zeros(my); 
%inser in xx all parameters
nparm=str.nparm; xx=zeros(1,nparm); xi=str.xi; 
j=0;
for i=1:nparm
 if xi(i) == 1
  j=j+1;
...
result.xvf= xvf; result.xf = xf; result.sigma2c = sigma2c; 
result.Sigmar = Sigmar; result.tv = tv; result.residexct = residexct; 
result.e = e; result.ff = ff; result.h = h; result.H = H; 
result.A = A; result.P = P;
if ~isempty(Y)
 %t-values of regression parameters
 H=H*sigma2c;
 tvr=h./sqrt(diag(H)); result.tvr=tvr;
else
 result.tvr=[];
end
