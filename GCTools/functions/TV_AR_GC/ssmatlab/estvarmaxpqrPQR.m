...
[nor,mor]=size(ordersr); [nos,mos]=size(orderss);
...
if nargin < 6
 hr3 = 1; finv2 = 0; mstainv = 0; nsig=[0 0]; tsig=[.75 1.];
end
if nargin < 7 
 finv2 = 0; mstainv = 0; nsig=[0 0]; tsig=[.75 1.];
end
if nargin < 8 
 mstainv = 0; nsig=[0 0]; tsig=[.75 1.];
end
if nargin < 9
 nsig=[0 0]; tsig=[.75 1.];
end
...
if (hr3 == 0) && (finv2 == 1) %third step of the HR method
 if str.nonst3 == 1
  [mphi,nphi,pp1]=size(str.phi); p=pp1-1;
  cont=0;
  for i=1:p
 ...
 if str.noninv3 == 1
  [mtheta,ntheta,qp1]=size(str.theta); q=qp1-1;
  cont=0;
  for i=1:q
