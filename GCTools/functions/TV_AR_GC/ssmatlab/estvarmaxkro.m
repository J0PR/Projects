...
% PURPOSE: estimates a VARMAX model in echelon form using the Hannan-Rissanen 
...
%          reestimate the model using the MHANRIS function.
%---------------------------------------------------
% USAGE: [str,ferror] = estvarmaxkro(y,x,seas,kro,hr3,finv2,mstainv,nsig,tsig)
% where:    y      = an (nobs x neqs) matrix of y-vectors
%           x      = matrix of input variables (nobs x nx)
%                   (NOTE: constant vector automatically included)
%        seas      = seasonality
%         kro      = an (1 x neqs) array containing the Kronecker indices
...
if ~isempty(x)
 ...
 if (nx ~= ny)
  ferror=1;
  disp('estvarmaxkro: nobs in x-matrix not the same as y-matrix');
 end
else
 m=0;
end
...
%create echelon structure for estimation
str = matechelon(kro,s,m);  
%estimate echelon form model by Hannan-Rissanen method
...
if (hr3 == 0) && (finv2 == 1) %third step of the HR method
 ...
 nlag=max(kro);
 if (nlag > 0)
  str3.thetast=str.thetast3; str3.phist=str.phist3; 
  str3.gammast=str.gammast3; str3.mu=str.mus3; str3.phis=str3.phis3;
  [resid3,sigmar3]=compresde0(y,x,str3);
 else
  resid3=str.resid3; sigmar3=str.sigmar3;
 end
 str.resid3m=resid3; str.sigmar3m=sigmar3;  
