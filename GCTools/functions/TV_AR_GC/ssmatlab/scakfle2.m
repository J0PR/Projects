function [e,f,hb,Mb,A,P,qyy,R]=scakfle2(y,X,Z,G,W,T,H,ins,i,chb)
%
%
...
%end of modification      
      ...
      error('singular matrix different from zero in scakfle1')
     end
    elseif miss == p
%modified 19th Nov. 2004      
%       DD=zeros(p,ndb1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     if ndelta > 0
%      [Q,SQTp]=qr([DD(:,1:end-1); SQTs(1:ndb,1:end-1)]);
%      SQTs=[SQTp Q'*[DD(:,end); SQTs(1:ndb,end)]]; 
%      SQT=[SQT;DD];
%     else
%      SQT=[SQT;DD];
%     end
...
%    pause
   ...
   if collps == 0 & ns >= nd  
    error('No collapsing is possible in scakfle2')
   end
%   f=f^(1/(nmiss-nd));
...
%added 21st Feb. 2012  
     if ms < nbeta
      error('Singular model in SCAKFLE2')
     end
%end of addition      
     [Q,R]=qr(SQT(:,1:nbeta));
     qy=Q'*SQT(:,nbeta+1); 
     ...
     e=qy(nbeta+1:ns); 
     if chb == 1
      %the following lines modified 18th. April 2013
