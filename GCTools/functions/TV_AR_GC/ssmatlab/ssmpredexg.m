...
%      stx  = a structure. If it is empty, the inputs are stochastic and
...
F=stx.T; H=stx.Z; B=stx.B; D=stx.D; m1=stx.m1;
[neqs,nalpha]=size(H);
inc= 1; krov=stx.kro;
...
yx=reshape(yfvec,neqs,nobs); V=yx';   %transform from vector to matrix
...
if isempty(sts)
 ...
 Fz=sts.T; Hz=sts.Z; Kz=sts.H; Jz=sts.G; W=[]; X=[];
 [mx,ngamma]=size(Hz);
 %set up combined model
 T=[F B*Hz; zeros(ngamma,nalpha) Fz]; HH=[B*Jz; Kz];  
 Z=[H D*Hz]; G=D*Jz;
 %initial conditions
