...
% A1
% A2
P1=phi(:,:,1); Psi0=eye(np);   %compute the necessary Psi(z)=Phi^{-1}(z) weights
Psi=-phi(:,:,2); if (ieyep == 0), Psi0=P1\Psi0; Psi=P1\Psi*Psi0; end  %first weight
for i=2:p                                         %rest of the weights
 A=[];
 for j=i:-1:2
  A=[A phi(:,:,j)];
 end
 AA=-phi(:,:,i+1); if (ieyep == 0), AA=AA*Psi0; end
 Psix= AA-A*Psi; 
 if (ieyep == 0), Psix=P1\Psix; end
 Psi=[Psi; Psix];
end
% Psi 
psi=phi; psi(:,:,1)=Psi0;
...
s2=s*s;
nt11=(p-n)*s2; nt22=(n+1)*s2;
T11=zeros(nt11,nt11); T21=zeros(nt22,nt11);
T22=zeros(nt22,nt22); H12=zeros(nt11,nt22);
H21=zeros(nt22,nt11); H22=zeros(nt22,nt22);

bphi=zeros(s2,s2,p+1); fbphi=zeros(s2,s2,p+1);
Is2=sparse(eye(s2)); Is=sparse(eye(s));
W=[];
for i=1:s
%  W=[W kron(Is,Is(:,i))];
 W=[W kIv(Is(:,i))];
...
T11=T(1:nt11,1:nt11); H12=H(1:nt11,nt11+1:end); clear T H
T=zeros((n+1)*s2); H=T;
for i=p-n+1:p+1
 for j=1:i
  T((i-1)*s2+1:i*s2,(j-1)*s2+1:j*s2)=bphi(:,:,i-j+1);
...
T21=T(nt11+1:end,1:nt11); T22=T(nt11+1:end,nt11+1:end);
H21=H(nt11+1:end,1:nt11); H22=H(nt11+1:end,nt11+1:end); clear T H
% T11
% T21
% T22
% H12
...
D12=T11*H12; D21=T22*H21; D22=T21*H12+T22*H22; C1=T11*A1; 
C2=T21*A1+T22*A2;
% D12
% D21
% D22
...
AX2=eye(nt22)+D22-D21*D12;  BX2=C2-D21*C1;
% AX2
% BX2
AX2=sparse(AX2); BX2=sparse(BX2);
% if (ieyep == 1)
