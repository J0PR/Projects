...
if mx > p | mz > p | mg > p | mw > nalpha | mt > nalpha | mh > nalpha
   ... | mu > mucd | mmc > mucd | md > mucd
%
...
AHD=zeros(nd,ndb1);
FST=zeros(n*p,p);
nkkp=max(nalpha,mucd); KKP=zeros(n*p,nkkp);
AAT=zeros((n+1)*nalpha,ndb1);
PT=zeros((n+1)*nkkp,nkkp);  %ojo cuando nalpha < mucd
...
%      FST(ip,:)=J'*Lm1*J;
    ...
    first=i*nkkp; iam=first+1:first+nalpha;    
...
%   i
    ...
    first=(i-1)*nkkp; iam=first+1:first+nalpha; iamm=first+1:first+mucd;
    if mz > p 
       ZZ=Z(ip,:);
    else
       ZZ=Z;
...
%     end
    ...
    CKZ=(CC-Kjj*Lm1*ZZ); DKG=(DD-Kjj*Lm1*GG);
    Pjjm1=CKZ*P*TT'+DKG*HH'; Pjj=CKZ*P*CC'+DKG*DD'; 
    if isempty(UU)
...
%     end
    ...
    K=KKP(ip,1:nalpha)';
    A=AAT(ia,:);
% The following line changed on December 24th 2007. It contained an error
    P=PT(iam,1:nalpha); 
% end of modification    
    L=(TT-K*ZZ);
    Kjj=(CC*P*ZZ'+DD*GG')*Lm1';
...
%
    if nd > 0 & iti < n
     ...
     ia=i*nalpha+1:(i+1)*nalpha;AA=AAT(ia,:); 
...
for i=1:n
 im=(i-1)*mucd+1:i*mucd; 
 first=(i-1)*nkkp; iamm=first+1:first+mucd;
