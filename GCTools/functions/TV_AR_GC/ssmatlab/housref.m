...
%
[m,n]=size(A); ieyeq=0; ierror=0; R=[]; Indx=[];
if nargin < 2
 Q=[];
else
 [nq,mq]=size(Q); 
...
for i=1:n
 if j <= m
  a=A(j:end,i); [na,ma]=size(a); Indx(i)=0;
  [q,r]=jqrt(a,na,0); Qta=qtb(q,A(j:end,i:end),na,0); 
  A(j:end,i:end)=Qta; 
  if ieyeq == 1
   Qtq=qtb(q,eye(na),na,0); Q2=Q1; 
   Q2(end-na+1:end,end-na+1:end)=Qtq; Q=Q2*Q;
  end
  if jnorm(r,na,0) <= tol
   Indx(i)=1;  
  else 
