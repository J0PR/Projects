...
nr=length(rt);
nalpha=np+ns+nr;
npm1=np-1; nsm1=ns-1; nrm1=nr-1;
nppns=np+ns;
% matrix T
Tp=zeros(np);  
Tp(1:npm1,2:end)=eye(npm1); Tp(np,2:np)=-ptden(1:end-1);
Ts=zeros(ns); 
...
T=zeros(nalpha);
T(1:np,1:np)=Tp; T(np+1:nppns,np+1:nppns)=Ts;
if nr > 0
 nppnspnr=nppns+nr;
...
H(1:np,1)=psip'; H(np+1:nppns,2)=psis';  
if (nr > 0)
 psir=fliplr(rt);
...
% matrices Z, W and X
Z=zeros(1,nalpha); Z(1)=1.; Z(np+1)=1.;
if (nr > 0)
 Z(nppns+1)=1.;
end
W=[];  
