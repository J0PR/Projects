...
phi=[]; theta=[]; gamma = []; ierror=0;
[p1,p2,p3]=size(phie); [t1,t2,t3]=size(thetae); [s1,s2,s3]=size(gammae);
if ( p1 ~= p2 ) | ( p1 ~= t1 )
...
phi=phie; theta=thetae; gamma=gammae; 
phi0=phie(:,:,1); inv=0;
...
if inv == 1
 ...
 for i=2:p3
  phi(:,:,i)=phi0\phie(:,:,i);
  theta(:,:,i)=phi0\thetae(:,:,i);
  if (s2 > 0)
   gamma(:,:,i)=phi0\gammae(:,:,i);
  end
 end
end
phi(:,:,1)=eye(p1); theta(:,:,1)=eye(t1); 
