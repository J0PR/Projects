...
str.s=np; str.freq=freq;
p=pm1-1; q=qm1-1; P=Pm1-1; Q=Qm1-1;
str.phi=phi; str.th=th; str.Phi=Phi; str.Th=Th; str.Sigma=Sigma;
...
phin=phi; phin(:,:,2:end)=NaN(size(phi(:,:,2:end)));
thn=th; thn(:,:,2:end)=NaN(size(th(:,:,2:end)));
Phin=Phi; Phin(:,:,2:end)=NaN(size(Phi(:,:,2:end)));
Thn=Th; Thn(:,:,2:end)=NaN(size(Th(:,:,2:end)));
str.phin=phin; str.Phin=Phin; str.thn=thn; str.Thn=Thn;
Lhn=Lh; Lhn(2:end)=NaN(size(Lh(2:end)));                           
str.Lhn=Lhn;
...
nparm=length(x);
%  x
str.x=x; str.nparm=nparm;  
xi=ones(size(x)); xv=x; xf=[];
str.xi=xi; str.xv=xv; str.xf=xf;

%transform multiplicative to non-multiplicative model
[phirs,thrs,H,F,G,J,ferror] = varmapqPQ2ssm(phi,th,Phi,Th,L,str);

str.phirs=phirs; str.thrs=thrs;
str.Z=H; str.T=F; str.H=G; str.G=J;
str.X=[]; str.W=[];