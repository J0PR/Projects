...
for i=1:s
 for p=1:i-1
  for k=kro(i)+2-npar(i,p):kro(i)+1
   phi(i,p,k)=NaN;  nparm=nparm+1;
  end
 end
 for k=2:kro(i)+1
  phi(i,i,k)=NaN; nparm=nparm+1;
 end
 for p=i+1:s
  for k=kro(i)+2-npar(i,p):kro(i)+1
   phi(i,p,k)=NaN;  nparm=nparm+1;
...
F=zeros(n,n); H=zeros(s,n); K=NaN(n,s);      % system matrices
B=NaN(n,m); D=NaN(s,m);
ch=0;
for i=1:s
 if kro(i) > 0
