...
Z=models.Z; T=models.T; G=models.G; H=models.H; X=models.X;  
stord=models.stord; conc=models.conc; trend=models.trend;
slope=models.slope; seas=models.seas; irreg=models.irreg;
arp=models.arp; nalphao=models.nalphao;
...
[nh,mh]=size(H); 
H(1,1)=xp(1);
H(2,1)=xp(2); H(2,2)=xp(3); H(3,3)=xp(4); H(4,3)=xp(5); H(4,4)=xp(6);
j=5;
for i=6:2:nh
  H(i,j)=xp(8); H(i-1,j)=xp(7); H(i,j+1)=xp(9);
  j=j+2;
end
% for i=6:2:nh
%  H(i,5)=xp(8); H(i-1,5)=xp(7); H(i,6)=xp(9);
% end
G(1,mh-1)=xp(10); G(2,mh-1)=xp(11); G(2,mh)=xp(12); 
...
% stacked model
%
Fp=eye(nalpha); G_p=[]; H_p=[]; [nj,mj]=size(J); J_p=zeros(p*nj,p*mj);
if ~isempty(X)
...
for k=1:p
 Fpm1=Fp;
 Fp=Fp*F; G_p=[Fpm1*G G_p]; H_p=[H_p; H*Fpm1];
...
% J_p
% error('matrices en modstr')
H_pbar=D*H_p; J_pbar=D*J_p; 
if ~isempty(V_p)
 V_pbar=(D*V_p)'; X=V_pbar;
end
% Fp
% G_p
% H_p
...
%  error('prueba')
 nalphao=sum(k); models.nalphao=nalphao;
end
Fno= Q*Fp*Q'; Gno= Q*G_p; Hno = H_pbar*Q';
% nalphao=5;
%
% low-frequency model
%
first=nalpha-nalphao+1;
T=Fno(first:nalpha,first:nalpha); H=Gno(first:nalpha,:); 
G=J_pbar; Z=Hno(:,first:nalpha);
% T
% H
% G
% Z
W=[]; C=[]; A=eye(nalphao); 
[nC,junk]=size(C); [junk,nA]=size(A);
ins=[C A]; i=[nC 0 0 nA];

modelsn.Z=Z; modelsn.T=T; modelsn.X=X; modelsn.W=W; modelsn.G=G;
modelsn.H=H; modelsn.ins=ins; modelsn.i=i;
modelsn.stord=stord; modelsn.conc=conc; modelsn.trend=trend;
modelsn.slope=slope; modelsn.seas=seas; modelsn.irreg=irreg;
modelsn.arp=arp; modelsn.nalphao=nalphao;
