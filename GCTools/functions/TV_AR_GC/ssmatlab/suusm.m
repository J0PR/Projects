...
if isfield(comp,'datei')
 ...
 disp('field datei should be in structure comp in suusm');
 ferror=1;
end
...
if isfield(comp,'freq')
 ...
 if (freq~=1) && (freq~=4) && (freq~=12)
     disp('freq should be one, four and twelve')
     ferror=1;
 end
 else
 disp('field datei should be in structure comp in suusm');
...
if isfield(comp,'irreg')
 ...
 if ~strcmp(conout,'irreg')
  x=[x irreg]; stord=[stord 1]; j=j+1;
  if isnan(irregn)
   xv=[xv irreg]; pvar=[pvar j];
  else
   xf=[xf irreg]; pfix=[pfix j];
  end
 else
  if ~isnan(irregn)
...
if isfield(comp,'level')
 ...
 if levelt > 0
     ...
     if ~strcmp(conout,'level')
        x=[x level]; stord=[stord 2]; j=j+1;
        if isnan(leveln)
            xv=[xv level]; pvar=[pvar j];
        else
            xf=[xf level]; pfix=[pfix j];
        end
     else
        if ~isnan(leveln)
...
if isfield(comp,'slope')
  ...
  if slopet == 1
    if ~strcmp(conout,'slope')
     x=[x slope]; stord=[stord 3]; j=j+1;
     if isnan(slopen)
        xv=[xv slope]; pvar=[pvar j];
     else
        xf=[xf slope]; pfix=[pfix j];
     end
    else
     if ~isnan(slopen)
...
if isfield(comp,'seas')
     if seast > 0
        if ~strcmp(conout,'seas')
          x=[x seas]; stord=[stord 4]; j=j+1;
          if isnan(seasn)
             xv=[xv seas]; pvar=[pvar j];
          else
            xf=[xf seas]; pfix=[pfix j];
          end
        else
          if ~isnan(seasn)
...
if isfield(comp,'ar')
    ...
    if ~strcmp(conout,'ar')
       x=[x ar]; stord=[stord 5]; j=j+1;
       if isnan(arn)
          xv=[xv ar]; pvar=[pvar j];
       else
          xf=[xf ar]; pfix=[pfix j];
       end
    else
       if ~isnan(arn)
 ...
 %end of modification
    ...
    for i=1:narp
        jj=jj+1; stord=[stord jj]; j=j+1;
        if isnan(arpn(i))
           xv = [xv arp(i)]; pvar=[pvar j];
        else
           xf = [xf arp(i)]; pfix=[pfix j];
        end
     end
end
...
if isfield(comp,'cycle')
   ...
   if ~strcmp(conout,'cycle')
      x=[x cycle]; stord=[stord 6]; j=j+1;
      if isnan(cyclen)
         xv=[xv cycle]; pvar=[pvar j];
      else
         xf=[xf cycle]; pfix=[pfix j];
      end
   else
      if ~isnan(cyclen)
   ...
   cyclep1=cyclep(1); cyclep2=cyclep(2);
   ...
   for i=1:ncyclep
       jj=jj+1; stord=[stord jj]; j=j+1;
       if isnan(cyclepn(i))
          xv = [xv cyclep(i)]; pvar=[pvar j];
       else
          xf = [xf cyclep(i)]; pfix=[pfix j];
       end
   end
end
...
%**********************************************************************
modescr.trend=levelt; modescr.slope=slopet; modescr.seas=seast;
modescr.cycle=cyclet;
if cyclet > 0
 modescr.xl1=xl1; modescr.xl2=xl2;
end
modescr.ar=art; modescr.irreg=irregt; 
modescr.stord=stord; modescr.conc=conc;

str=modelstruc(xv,y,Y,pfix,pvar,xf,modescr,datei,npr);
str.x=x; str.xv=xv; str.xf=xf; str.pfix=pfix; str.pvar=pvar;
str.freq=freq; str.datei=datei; str.comp=comp;