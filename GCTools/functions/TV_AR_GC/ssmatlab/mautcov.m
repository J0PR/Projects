...
str.c0=c0; str.cv=cv; str.r=r; str.sgn=sgn; str.sgnt=sgnt;
...
if nargin == 4
 ...
 r0i=r0\eye(size(r0)); V=kron(r0i,r0i);
 qstat=zeros(lag,1); sr=zeros(lag,1); 
 df=zeros(lag,1); pval=zeros(lag,1);
 for i=1:lag
  rv=vec(r(:,:,i)');
  sr(i)=rv'*V*rv;
