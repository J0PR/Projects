...
thrc=[]; thsc=[]; sigma2r=[]; sigma2s=[]; thtc=[]; sigma2t=[];
ierrcandec=0;
...
dunum=length(unum)-1; 
dudenr=length(udenr)-1; dudens=length(udens)-1; duden=dudenr+dudens;
if dunum >= duden
 [uq,ur]=deconv(unum,uden); dur=dunum-1; ur=ur(2:end);
else
 uq=0.; ur=unum; dur=dunum;
end
...
udenrm=zeros(1,1,dudenr+1); udenrm(:,:,1:end)=fliplr(udenr);
udensm=zeros(1,1,dudens+1); udensm(:,:,1:end)=fliplr(udens);
...
[nu,mu,pu]=size(U);
unr=zeros(1,pu); uns=zeros(1,pu);
unr(1:end)=U(1,1,:);  uns(1:end)=U(2,1,:); 
unr=fliplr(unr)./G; uns=fliplr(uns)./G; %polynomials are normalized
...
tol=1d-14;
[q,rs]=deconv(a,udens); rs=cleanpol(rs,tol);  %rs is the numerator of udens
[qq,rr]=deconv(b,udenr); rr=cleanpol(rr,tol); %rr is the numerator of udenr
