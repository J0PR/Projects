...
figure
trend=Xt(:,i);
trendpcw=trend+cw*strend(:,i); trendmcw=trend-cw*strend(:,i);
vnames=strvcat(['trend' num2str(i) '+.95mse'],['series' num2str(i) ],...
...
%model matrices
X=modelsf.X; Z=modelsf.Z; G=modelsf.G; W=modelsf.W;
T=modelsf.T; H=modelsf.H; ins=modelsf.ins; ii=modelsf.i; 


%extend series with one year NaNs for forecasting
npr=16;
yf=NaN(npr,my); [ny,p]=size(y);
y=[y ; yf];
...
xlswrite('results\viviusaf.xls',Xsct);
...
iobs=ical(2011,6,datei); fobs=ical(2014,1,datei); 
bg_yr1 = 2011;  bg_per1 = 6;
datei2 = cal(bg_yr1,bg_per1,freqm);
for i=1:my
figure
trend=Xsct(iobs:fobs,i); ser=y(iobs:fobs,i);
%uncomment next line if there are regression variables
trend=trend + X((iobs-1)*p+i:p:(fobs-1)*p+i,:)*gsc(end-nxpr+1:end);
vnames=vertcat(titles(i),strcat(titles(i), ' + 16 Forecasts'));
hold on
