function GGC=CPGewekeGC(X,B,A,C,NLAGS,N,Fs)
%CPGewekeGC so sub-model is computed with correlation purged AR model...
if strcmp(B,'np')
    [K1,K2,N] = size(A);
    h=A;
    if K1==2
...
if K1>2
    GGC=conditionalGC(h,C,X,Fs,NLAGS,N,K2,K1,B);
else
    for k1=1:K1;
        for k2=1:K2;
...
function GC=conditionalGC(condH,condC,data,Fs,NLAGS,N,from,to,np_FLAG)
