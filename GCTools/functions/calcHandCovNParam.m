function [H, Z, S, freq]=calcHandCovNParam(X,Fs)
...
%use try catch to compile to get a mex file..
% try
%    [H, Z, S, psi] = wilsonSpectralFactor_mex(S);
% catch exception
%    disp('Compiling wilsonSpectralFactor')
%    codegen sfactorization_wilson -args {S} 
%    [H, Z, S, psi] = wilsonSpectralFactor_mex(S);
% end
...
for iter = 1:Niterations
  ...
  for k = 1:N2
    psi(:,:,k) = psi(:,:,k)*gp(:,:,k);
    psierr(k,iter)  = norm(psi(:,:,k)-psi_old(:,:,k),1);
  end
  if sum(isnan(psi(:))) ~= 0
      psierrf = mean(psierr(:,iter));
  end
  if checkflag,
    psierrf(iter) = mean(psierr(:,iter));
    if(psierrf<tol), 
