...
void corr(int row, int col, double *v, double *r) {
    int i, j, k;
    double *vm=new double[col], *sd=new double[col];
    for (i=0;i<col;i++) meanstd(row, &v[i*row], &vm[i], &sd[i]);
    for(i=0;i<col;i++){
        r[i*col+i]=1;
        for(j=i+1;j<col;j++) {
            for(k=0;k<row;k++) r[i*col+j]+=(v[i*row+k]-vm[i])/sd[i]*(v[j*row+k]-vm[j])/sd[j];
            r[i*col+j]/=(row-1);
            r[j*col+i]=r[i*col+j];
        }
    }
...
double MI_gaussian(double *Y,double *X,int N,int ky,int kx){
    ...
    px=Determinant(x_cov, kx);
    py=Determinant(y_cov, ky);
    if(pxy>0) te=0.5*log(px*py/pxy);
    delete[] x_cov;
    delete[] y_cov;
    delete[] XY;
    delete[] xy_cov;
